...
def build_context(params: dict) -> str:
    competencies_str = ", ".join(params.get("competencies", []))
    return f"""## 시뮬레이션 설계 조건

- 평가 목적: {params.get('evaluation_purpose', '역량 진단')}
- 대상 직급: {params.get('target_level', '과장')}
- 산업/업종: {params.get('industry', 'IT/소프트웨어')}
- 직무: {params.get('job_function', '경영기획')}
